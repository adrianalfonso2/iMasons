...
export default function MentorshipCreatePage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (isEdit) {
      api.get(`/mentorships/${id}`).then((data) => {
        setForm({
          title: data.title,
      ...
      }).catch(() => setError('Failed to load mentorship'));
    }
  }, [id, isEdit]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await api.put(`/mentorships/${id}`, form);
      } else {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">
        {isEdit ? 'Edit Mentorship Posting' : 'Create Mentorship Posting'}
      </h1>
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="e.g., Cloud Architecture Mentor"
          />
        </div>
        <div>
          ...
          <textarea
            ...
            rows={6}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-green-500"
            placeholder="Describe the mentorship opportunity and expectations..."
          />
        </div>
