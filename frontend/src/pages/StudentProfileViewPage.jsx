...
export default function StudentProfileViewPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <Link
        to="/students"
        className="inline-flex items-center gap-1 text-brand-teal hover:text-brand-purple text-sm font-medium mb-6 transition-colors"
      >
        ← Back to Students
      </Link>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
        ...
        {student.bio && (
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wide text-brand-teal mb-2">About</h3>
            <p className="text-gray-700 whitespace-pre-wrap text-sm leading-relaxed">{student.bio}</p>
          </div>
        )}

        {skills.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wide text-brand-teal mb-3">Skills</h3>
            <div className="flex flex-wrap gap-2">
              {skills.map((skill) => (
                <span
                  key={skill}
                  className="bg-brand-purple/10 text-brand-purple border border-brand-purple/20 rounded-full px-3 py-1 text-sm font-medium"
                >
                  {skill}
                </span>
              ))}
        ...
        {(student.resumeLink || student.linkedinUrl || student.githubUrl || student.portfolioUrl) && (
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wide text-brand-teal mb-3">Links</h3>
            <div className="space-y-2">
              {student.resumeLink && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-2 text-brand-teal hover:text-brand-purple font-medium text-sm transition-colors"
                >
                  {/* Document icon */}
